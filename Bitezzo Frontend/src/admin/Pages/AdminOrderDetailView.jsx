...
import {
  ArrowLeft, MapPin, Package, Calendar, CreditCard, User, Phone, Mail, Clock,
  CheckCircle, Truck, XCircle, AlertCircle, DollarSign, Hash
} from 'lucide-react';
...
function AdminOrderDetailView() {
  ...
  // Defensive fallback for order arrays and address
  const items = order?.items ?? [];
  const address = order?.address ?? {};
  const totalItems = items.reduce((sum, product) => sum + (product.quantity ?? 0), 0);

  // Handle status change: API first, then update UI
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-lg font-semibold text-gray-900 break-all">#{order._id}</p>
          ...
          <div className="xl:col-span-1 space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                {[
                  { icon: User, label: 'Name', value: address.name },
                  { icon: MapPin, label: 'Street', value: address.street },
                  { icon: MapPin, label: 'City', value: address.city },
                  { icon: Hash, label: 'Pincode', value: address.pincode },
                  { icon: Phone, label: 'Phone', value: address.phone },
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-900">{order.payment ?? 'N/A'}</p>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                  ...
                  Ordered Items ({items.length} products, {totalItems} items)
                </h3>
              </div>
              <div className="p-6">
                <div className="space-y-4">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-shrink-0 text-right">
                          ...
                          <p className="text-lg font-bold text-green-600">
                            ₹{(price * (product.quantity ?? 0)).toLocaleString('en-IN')}
                          </p>
                        </div>
                      </div>
                    );
                  ...
                  <div className="bg-gradient-to-r from-gray-50 to-slate-50 rounded-lg p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-green-600">
                        ₹{parseFloat(order.total ?? order.subTotal ?? 0).toLocaleString('en-IN')}
                      </span>
                    </div>
                  </div>
                </div>
